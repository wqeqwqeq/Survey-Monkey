...
def missing_value_en(df,year,company):
    ...
    COMPANY_TABLE['email']=COMPANY_TABLE['COMPANY NAME'].apply(lambda x:'{}@gmail.com'.format(x.lower().replace(' ','')))
    COMPANY_TABLE['first_name'] = " " ## dont have enough information for their names
    COMPANY_TABLE['last_name'] = " "  ## thus, just leave it blank at this moment
    ...
    # Generate unique dataframe
    df =df[((df['COMPANY NAME']==company) & (df['YEAR']==year))].drop_duplicates()
    selected=df[((df.VALUE.isnull()) | (df.VALUE == 'Unclear') | (df.VALUE == 'Unknown'))]
    ...
    return selected, questions_list, recipients_df
...
def result_table(selected_df,df_result_answer, questions_list):
    ...
    questions_list = pd.DataFrame(questions_list)
    selected_df = selected_df[['COMPANY NAME','YEAR','SURVEY QUESTIONS','QUESTIONID']]
    result_table = selected_df.merge(questions_list,left_on=['SURVEY QUESTIONS'],right_on=[1],how='right')
    result_table = result_table.drop([0,1], axis =1)

    result_table = result_table.merge(df_result_answer,on=['SURVEY QUESTIONS'], how='left')
